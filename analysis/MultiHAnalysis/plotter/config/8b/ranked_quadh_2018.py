...
abcd_regions = {
  ...
  'valid_btag_hi':         [(3,5,9),(0,0.2 ,1)],
  'valid_btag_lo':         [(0,3,5),(0,0.2 ,1)],
  'valid_score_lo':        [(0,5,9),(0,0.15,0.2)],
  'valid_btag_hi_score_lo':[(3,5,9),(0,0.15,0.2)],
  'valid_btag_lo_score_lo':[(0,3,5),(0,0.15,0.2)]
}

def get_region(name, v1_r, v2_r):
    btag_sr = 'n_medium_btag >= %s && n_medium_btag < %s' % (v1_r[1], v1_r[2])
    btag_cr = 'n_medium_btag >= %s && n_medium_btag < %s' % (v1_r[0], v1_r[1])

    score_sr = 'quadh_score >=  %s && quadh_score < %s' % (v2_r[1], v2_r[2])
    score_cr = 'quadh_score >=  %s && quadh_score < %s' % (v2_r[0], v2_r[1])
    
    region = {
        'A':'%s && %s' % (btag_sr, score_sr),
        'B':'%s && %s' % (btag_cr, score_sr),
        'C':'%s && %s' % (btag_sr, score_cr),
        'D':'%s && %s' % (btag_cr, score_cr)
    }
    if name is not 'nominal':
        region = { name+'_'+key:value for key,value in region.items() }
    return region
...
for key, (v1_r, v2_r) in abcd_regions.items():
    selections_defs.update( **get_region(key, v1_r, v2_r) )
