...
def main(args):
    ...
    f = ROOT.TFile.Open("histogramsForHHTo4b_16Jan2023.root", "READ")
    ...
    # Make 2D histograms
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    histos2D = ["h2D_NBQuarksMatchedTo_GenJetsVsGenFatJets",
                "h2D_NBQuarksMatchedTo_RecoJetsVsRecoFatJets",
                "h2D_NBQuarksMatchedTo_RecoJetsVsCategory",
                "h2D_NBQuarksMatchedTo_RecoFatJetsVsCategory"
            ]
    ...
    for sample in samples:
        for h in histos2D:
            ...
            if "GenJetsVsGenFatJets" in h or "RecoJetsVsRecoFatJets" in h:
                ...
                for ibiny in range(1, histo.GetNbinsY()+1):
                    label = histo.GetYaxis().GetBinLabel(ibiny)
                    sumEvents = 0.0
                    for ibinx in range(1, histo.GetNbinsX()+1):
                        content = histo.GetBinContent(ibinx, ibiny)
                        sumEvents += content
                    for ibinx in range(1, histo.GetNbinsX()+1):
                        ...
                        if content == 0:
                            ...
                            histo.SetBinContent(ibinx, ibiny, float(content)/float(sumEvents)*100)
    ...
    histos = ["h_GenPart_H1_pt",
              ...
              "h_GenPart_H2_b1_eta",
              "h_GenPart_H2_b2_eta",
              "h_GenMatchingTo4GenJets_Cases",
              "h_GenMatchingTo4RecoJets_Cases",
              "h_NJets",
              "h_Jet1Pt",
              "h_Jet2Pt",
              "h_Jet3Pt",
    ...
    for sample in samples:
        for h in histos:
            ...
            if "h_N" in h:
                ...
                #print("\n Looking at histogram %s" % (h))
                histo.GetYaxis().SetTitle("Percentage of Events (%)")
                for ibin in range(1, histo.GetNbinsX()+1):
                    #print("ibin = ", ibin, "   content=", histo.GetBinContent(ibin))
                    ratio = float(histo.GetBinContent(ibin)/histo.GetEntries())*100
                    histo.SetBinContent(ibin, float(histo.GetBinContent(ibin)/histo.GetEntries())*100)
            else:
    ...
    # Compare Gen-Particles: Resolved, Resolved case 1, Resolved case 10, case 6, 7, 13
    ...
    for sample in samples:
        for h in histos:
            ...
            hResolved_inclusive = f.Get(sample+"/hResolved_"+h)
            hResolved_inclusive.GetYaxis().SetTitle("Arbitrary Units")
            if "RecoJet_N" in h:
                hResolved_inclusive = NormalizeToUnityTH1I(hResolved_inclusive)
            else:
                hResolved_inclusive.Scale(1.0/hResolved_inclusive.Integral())
                hResolved_inclusive.Rebin(2)
            if hResolved_inclusive.GetMaximum() > maxY:
                maxY = hResolved_inclusive.GetMaximum()
            hResolved_inclusive.SetTitle("")
            hResolved_inclusive.SetLineColor(ROOT.kBlue)
            hResolved_inclusive.SetMarkerColor(ROOT.kBlue)
            hResolved_inclusive.SetMarkerSize(1.05)
            hResolved_inclusive.SetFillColorAlpha(ROOT.kBlue, 0.75)
            hResolved_inclusive.SetFillStyle(3001)
            
            hResolved_Case1 = f.Get(sample+"/hResolvedCase1_"+h)
            if "RecoJet_N" in h:
                hResolved_Case1 = NormalizeToUnityTH1I(hResolved_Case1)
            else:
                hResolved_Case1.Scale(1.0/hResolved_Case1.Integral())
                hResolved_Case1.Rebin(2)
            if hResolved_Case1.GetMaximum() > maxY:
                maxY = hResolved_Case1.GetMaximum()
            hResolved_Case1.SetLineColor(ROOT.kBlue+2)
            hResolved_Case1.SetMarkerColor(ROOT.kBlue+2)
            hResolved_Case1.SetLineWidth(2)
            hResolved_Case1.SetMarkerSize(1.05)
            
            
            hResolved_Case6 = f.Get(sample+"/hResolvedCase6_"+h)
            if "RecoJet_N" in h:
                hResolved_Case6 = NormalizeToUnityTH1I(hResolved_Case6)
            else:
                hResolved_Case6.Scale(1.0/hResolved_Case6.Integral())
                hResolved_Case6.Rebin(2)
            if hResolved_Case6.GetMaximum() > maxY:
                maxY = hResolved_Case6.GetMaximum()
            hResolved_Case6.SetLineColor(ROOT.kOrange+2)
            hResolved_Case6.SetMarkerColor(ROOT.kOrange+2)
            hResolved_Case6.SetFillColorAlpha(ROOT.kOrange+2, 0.65)
            hResolved_Case6.SetFillStyle(3001)
            
            hResolved_Case7 = f.Get(sample+"/hResolvedCase7_"+h)
            if "RecoJet_N" in h:
                hResolved_Case7 = NormalizeToUnityTH1I(hResolved_Case7)
            else:
                hResolved_Case7.Scale(1.0/hResolved_Case7.Integral())
                hResolved_Case7.Rebin(2)
            if hResolved_Case7.GetMaximum() > maxY:
                maxY = hResolved_Case7.GetMaximum()
            hResolved_Case7.SetLineColor(ROOT.kRed+2)
            hResolved_Case7.SetMarkerColor(ROOT.kRed+2)
            hResolved_Case7.SetMarkerStyle(21)

            hResolved_Case10 = f.Get(sample+"/hResolvedCase10_"+h)
            if "RecoJet_N" in h:
                hResolved_Case10 = NormalizeToUnityTH1I(hResolved_Case10)
            else:
                hResolved_Case10.Scale(1.0/hResolved_Case10.Integral())
                hResolved_Case10.Rebin(2)
            if hResolved_Case10.GetMaximum() > maxY:
                maxY = hResolved_Case10.GetMaximum()
            hResolved_Case10.SetLineColor(ROOT.kGreen+2)
            hResolved_Case10.SetMarkerColor(ROOT.kGreen+2)
            hResolved_Case10.SetMarkerStyle(22)
            
            hResolved_Case13 = f.Get(sample+"/hResolvedCase13_"+h)
            if "RecoJet_N" in h:
                hResolved_Case13 = NormalizeToUnityTH1I(hResolved_Case13)
            else:
                hResolved_Case13.Scale(1.0/hResolved_Case13.Integral())
                hResolved_Case13.Rebin(5)
            if hResolved_Case13.GetMaximum() > maxY:
                maxY = hResolved_Case13.GetMaximum()
            hResolved_Case13.SetLineColor(ROOT.kGray+2)
            hResolved_Case13.SetMarkerColor(ROOT.kGray+2)
            hResolved_Case13.SetMarkerStyle(23)

            hResolved_Case13.SetMaximum(maxY*opts["ymaxfactor"])
            hResolved_inclusive.SetMaximum(maxY*opts["ymaxfactor"])
            
            hResolved_inclusive.Draw("hist")
            hResolved_Case1.Draw("hist same")
            hResolved_Case6.Draw("hist same")
            hResolved_Case7.Draw("same")
            hResolved_Case10.Draw("same")
            hResolved_Case13.Draw("same")
            
            hResolved_inclusive.GetXaxis().SetRangeUser(opts["xmin"], 800)
            if "b2_pt" in h or "b1_pt" in h:
                hResolved_inclusive.GetXaxis().SetRangeUser(opts["xmin"], 500)
            elif "_NFat" in h:
                hResolved_inclusive.GetXaxis().SetRangeUser(0, 6)
            elif "_NJets" in h:
                hResolved_inclusive.GetXaxis().SetRangeUser(0,12)

            legend.AddEntry(hResolved_inclusive, "resolved - inclusive")
            legend.AddEntry(hResolved_Case1, "case-1")
            legend.AddEntry(hResolved_Case6, "case-6")
            legend.AddEntry(hResolved_Case7, "case-7")
            legend.AddEntry(hResolved_Case10, "case-10")
            legend.AddEntry(hResolved_Case13, "case-13")
            legend.Draw("same")
            ...
            d.Update()
            for s in args.formats:
                saveName = os.path.join(args.path, "ComparingResolvedCases_%s_%s%s" % (h, sample, s))
                d.SaveAs(saveName)

    # Compare ResolvedExcl - Boosted - Semiresolved GenParticles
    histos = ["GenPart_H1_pt",
              "GenPart_H2_pt",
              "GenPart_H1_b1_pt",
              "GenPart_H1_b2_pt",
    ...
    # Now Compare Resolved - BoostedExcl - SemiresolvedExcl GenParticles
    for sample in samples:
        for h in histos:
            ROOT.gStyle.SetOptStat(0)
            ROOT.gStyle.SetTextFont(42)
            ...
            hSemi = f.Get(sample+"/hSemiresolvedExcl_"+h)
            if "H1_b2_pt" in h or "H2_b2_pt" in h:
            ...
            hBoosted = f.Get(sample+"/hBoostedExcl_"+h)
            hBoosted.Scale(1.0/hBoosted.Integral())
            if "H1_b2_pt" in h or "H2_b2_pt" in h:
            ...
            # Save plot
            for s in args.formats:
                saveName = os.path.join(args.path, "ResolvedFavored_%s_%s%s" % (h, sample, s))
                d.SaveAs(saveName)
    ...
    histos = [
        ...
        "hSemiresolvedExcl_H1Boosted_H2resolved_RecoJet_H2_b1_phi",
        "hSemiresolvedExcl_H1Boosted_H2resolved_RecoJet_H2_b2_phi",
        ...
        "hSemiresolvedExcl_H1Boosted_H2resolved_RecoFatJet_H1_subjet1_phi",
        ...
        "hSemiresolvedExcl_H1Boosted_H2resolved_RecoFatJet_H1_subjet2_phi",
        ...
        "hSemiresolvedExcl_H1Boosted_H2resolved_H2_phi",
