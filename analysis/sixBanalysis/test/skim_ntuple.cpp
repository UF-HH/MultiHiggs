...
int main(int argc, char** argv)
{
  ...
  desc.add_options()
    ...
    ("is-signal",     po::value<bool>()->zero_tokens()->implicit_value(true)->default_value(false), "mark as a signal sample (default is false)")
    //
  ...
  CfgParser config;
  if (!config.init(opts["cfg"].as<string>())) throw std::runtime_error("No config file was provided");
  ...
  if (nfiles == 0) throw std::runtime_error("The input file list is empty.");
  ...
  if (!is_data)
    {
      if (opts["puWeight"].as<string>().size() != 0)
	{ // a valid option is passed from cmd line
  ...
  for (int iEv = 0; true; ++iEv)
  {
    ...
    if ( doPickEvt && ((pickEvtNr >= 0 && (long long) *(nat.event) != pickEvtNr) ||
		       (pickRunNr >= 0 && (int) *(nat.run) != pickRunNr) ||
		       (pickLumiNr >= 0 && (long long) *(nat.luminosityBlock) != pickLumiNr) ) ) continue;
    ...
    // Apply trigger
    //====================================
    if (applyTrg && !(nat.getTrgOr())) continue;
